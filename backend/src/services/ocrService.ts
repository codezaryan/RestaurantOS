...
export class OCRService {
   ...
   */
  async processInvoice(filePath: string): Promise<ExtractedInvoiceData> {
    let rawText = '';
    let confidence = 0.88;
    ...
    const parsed = this.parseRawInvoiceText(rawText, path.basename(filePath));
    return {
      ...parsed,
   ...
   */
  private parseRawInvoiceText(text: string, filename: string): ExtractedInvoiceData {
    ...
    if (invMatch && invMatch[1]) {
      ...
      if (matchNum) invoiceNumber = `INV-2026-${matchNum[0]}`;
    }

    // 2. Supplier Name
    let supplierName = 'Metro Wholesale Foods Supply';
    if (upperText.includes('NILE') || upperText.includes('HOSPITALITY')) supplierName = 'Nile Hospitality Logistics';
    else if (upperText.includes('FRESH') || upperText.includes('FARM')) supplierName = 'Fresh Farms & Dairy Ltd';
    else if (upperText.includes('BAIKAL') || upperText.includes('SPHERE')) supplierName = 'Baikal Beverage Suppliers';
    else if (upperText.includes('OCEAN') || upperText.includes('SEAFOOD')) supplierName = 'Ocean Prime Seafood Co.';
    else {
      ...
      if (vendorLine) supplierName = vendorLine.slice(0, 35);
    }
