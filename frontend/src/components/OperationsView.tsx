...
import { Utensils, Clock, CheckCircle2, QrCode, Plus, ChevronRight, Flame } from 'lucide-react';
...
export const OperationsView: React.FC<OperationsViewProps> = ({
  ...
  const addItemToOrder = (menuItemId: string) => {
    ...
    if (existing) {
      setNewOrderItems(newOrderItems.map(i => i.menuItemId === menuItemId ? { ...i, quantity: i.quantity + 1 } : i));
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 glass-card p-5 rounded-2xl border border-slate-800">
        <div>
          <h1 className="text-xl font-extrabold text-white">Restaurant Operations & Kitchen Display (KDS)</h1>
          <p className="text-xs text-slate-400">Floor table management, live kitchen preparation board, and recipe menu catalog.</p>
        </div>
        ...
        <div className="flex items-center space-x-2 bg-slate-900/80 p-1 rounded-xl border border-slate-800">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${
              subTab === 'tables' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${
              subTab === 'kds' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-400 hover:text-white'
            }`}
          >
            <Flame className="w-3.5 h-3.5" />
            Kitchen Display System ({pendingOrders.length + preparingOrders.length})
          </button>
          <button
            onClick={() => setSubTab('menu')}
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${
              subTab === 'menu' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:text-white'
            }`}
          >
              ...
              return (
                <div
                  ...
                  className={`glass-card p-5 rounded-2xl border transition-all duration-200 relative ${
                    isOccupied ? 'border-amber-500/50 bg-amber-950/10' :
                    isReserved ? 'border-purple-500/50 bg-purple-950/10' :
                    isCleaning ? 'border-blue-500/50 bg-blue-950/10' :
                ...
                >
                  ...
                  {activeOrder && (
                    <div className="mt-3 p-2.5 rounded-xl bg-slate-900/80 border border-slate-800 text-xs">
                      <div className="flex justify-between font-semibold text-slate-200">
                        ...
                        <span className="text-amber-400">${activeOrder.totalAmount}</span>
                      </div>
                      <p className="text-[11px] text-slate-400 mt-0.5 truncate">
                        {activeOrder.orderItems.map(i => `${i.quantity}x ${i.menuItem?.name}`).join(', ')}
                      </p>
                    </div>
                  )}
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between gap-2">
                    <div className="flex items-center space-x-1">
                      <button
                        onClick={() => onUpdateTableStatus(t.id, t.status === 'AVAILABLE' ? 'OCCUPIED' : 'AVAILABLE')}
            ...
            <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="glass-card max-w-xl w-full p-6 rounded-2xl border border-slate-700 space-y-4">
                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                  <h3 className="text-lg font-bold text-white">Create New Order for Table {selectedTable.tableNumber}</h3>
                  <button onClick={() => setSelectedTable(null)} className="text-slate-400 hover:text-white">&times;</button>
                </div>
                  ...
                  {menuItems.map(item => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-[11px] text-emerald-400">${item.price}</p>
                      </div>
                ...
                {newOrderItems.length > 0 && (
                  <div className="bg-slate-900 p-3 rounded-xl border border-slate-800 space-y-1">
                    <p className="text-xs font-bold text-slate-300">Selected Items:</p>
                    {newOrderItems.map(i => {
                      const m = menuItems.find(x => x.id === i.menuItemId);
                      return (
                        <div key={i.menuItemId} className="flex justify-between text-xs text-slate-400">
                          <span>{i.quantity}x {m?.name}</span>
                          <span>${((m?.price || 0) * i.quantity).toFixed(2)}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="flex justify-end space-x-2 pt-2">
                  <button onClick={() => setSelectedTable(null)} className="px-4 py-2 rounded-xl bg-slate-800 text-xs font-bold text-slate-300">Cancel</button>
                  <button onClick={handleCreateOrderSubmit} className="px-5 py-2 rounded-xl bg-blue-600 text-xs font-bold text-white shadow-lg shadow-blue-500/20">Submit Order to Kitchen</button>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="glass-card p-4 rounded-2xl border border-slate-800 space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              <span className="text-sm font-bold text-amber-400 uppercase tracking-wider flex items-center gap-1.5">
                <Clock className="w-4 h-4" /> Pending Tickets ({pendingOrders.length})
              </span>
            </div>
            {pendingOrders.map(order => (
              <div key={order.id} className="bg-slate-900/90 p-4 rounded-xl border border-slate-800 space-y-2">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-extrabold text-white">#{order.orderNumber} &bull; {order.table?.tableNumber || 'Takeout'}</span>
          ...
          <div className="glass-card p-4 rounded-2xl border border-slate-800 space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              <span className="text-sm font-bold text-blue-400 uppercase tracking-wider flex items-center gap-1.5">
                <Flame className="w-4 h-4 animate-pulse" /> Cooking Station ({preparingOrders.length})
              </span>
            </div>
          ...
          <div className="glass-card p-4 rounded-2xl border border-slate-800 space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              <span className="text-sm font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4" /> Ready for Serving ({readyOrders.length})
              </span>
            </div>
            {readyOrders.map(order => (
              <div key={order.id} className="bg-slate-900/90 p-4 rounded-xl border border-slate-800 space-y-2">
                ...
                <button
                  onClick={() => onUpdateOrderStatus(order.id, 'SERVED', 'PAID')}
      ...
      {subTab === 'menu' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {menuItems.map(item => (
            <div key={item.id} className="glass-card p-4 rounded-2xl border border-slate-800 space-y-3">
              <img src={item.imageUrl} alt={item.name} className="w-full h-36 object-cover rounded-xl border border-slate-800" />
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-bold text-white text-base">{item.name}</h3>
                  <p className="text-xs text-slate-400">{item.category?.name || 'Main Course'}</p>
                </div>
              ...
              <div className="pt-2 border-t border-slate-800 text-xs space-y-1">
                <span className="font-bold text-slate-300">Recipe Ingredients Required:</span>
                {item.recipes && item.recipes.length > 0 ? (
                  item.recipes.map(r => (
                    <div key={r.id} className="flex justify-between text-slate-400 text-[11px]">
                      <span>&bull; {r.ingredient?.name}</span>
                      <span>{r.quantityRequired} {r.ingredient?.unit}</span>
                    </div>
                  ))
                ) : (
                  <p className="text-[11px] text-slate-500 italic">Standard kitchen prep recipe</p>
                )}
              </div>
            </div>
          ))}
